...
public class SEEUExtractClueweb09AnchorText {
	...
	public static class TokenizerMapper extends
		...
		private String html_schema = "trec";
		private String url_schema = "trec";
		...
		public String readDB(String id) throws IOException, SQLException,
			...
			String sql = "select html from " + this.html_schema + "." + this.html_tb
					+ " where id =\"" + id + "\"";
		...
		public String readURLDB(String id) throws IOException, SQLException,
			...
			String sql = "select url from " + this.url_schema + "." + this.url_tb
					+ " where id =\"" + id + "\"";
			System.out.println(sql);
			ResultSet rs = null;
			try {
		...
		public void query_html(ArrayList<String> subList,
				...
				IllegalAccessException, ClassNotFoundException, SQLException,
				IOException {
			String sql = "select id,html from " + this.html_schema + "."
		...
		public void query_urls(ArrayList<String> subList,
				...
				IllegalAccessException, ClassNotFoundException, SQLException,
				IOException {
			String sql = "select id,url from " + this.url_schema + "."
		...
		public void map(Object key, Text value, Context context)
			...
			html_schema = conf.get("html_schema");
			url_schema = conf.get("url_schema");
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 14) {			
		      System.err.println("Usage: Extract [-idurlmap <idurlmap>] [-html_schema <html_schema>] [-html_tb <html_tb>] [-url_schema <url_schema>] [-url_tb <url_tb>] [-idfolder <id_folder>] [-outfolder <outfolder>]");
		      return ;
		    }		  
		  String html_schema = "";		  
		  String idfolder = "";		  		  
		...
		// TODO Auto-generated method stub
		    for (int i = 0; i < otherArgs.length; i++) {
		    	if (otherArgs[i].equals("-html_schema")) {
		    		html_schema = otherArgs[++i];
		      ...
		      else if (otherArgs[i].equals("-url_schema")) {
		    	  url_schema = otherArgs[++i];
		      }
		...
		conf.set("html_schema", html_schema);
		conf.set("url_schema", url_schema);
		conf.set("html_tb", html_tb);
		conf.set("url_tb", url_tb);
		conf.set("idurlmap", idurlmap);
		
		Job job = new Job(conf, "seeu classification");
		job.setJarByClass(SEEUExtractClueweb09AnchorText.class);
